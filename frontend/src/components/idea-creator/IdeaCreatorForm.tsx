...
export const IdeaCreatorForm: React.FC<IdeaCreatorFormProps> = ({ onAnalysisCompleted }) => {
  ...
  const applyPreset = (presetType: 'farm' | 'ai') => {
    if (presetType === 'farm') {
      setFormData({
    ...
    return (
      <div className="max-w-3xl mx-auto py-20 px-4 text-center space-y-8 animate-fadeIn">
        ...
        <div className="space-y-3">
          <h2 className="text-2xl font-extrabold text-white">Validation Engine Synthesizing...</h2>
          <p className="text-sm text-warm-400">Synthesizing market signals, financial unit economics, and risk matrix</p>
        </div>

        {/* Animated Progress Steps List */}
        <div className="max-w-md mx-auto rounded-2xl glass-card bg-warm-900 border border-warm-800 p-6 space-y-3 text-left">
            ...
            return (
              <div key={idx} className="flex items-center space-x-3 text-xs">
                {isDone ? (
                  ...
                  <div className="w-5 h-5 rounded-full bg-warm-800 text-warm-500 flex items-center justify-center text-[10px]">
                    {idx + 1}
                  </div>
                )}
                <span className={isDone ? 'text-stone-200 font-semibold' : isCurrent ? 'text-emerald-300 font-bold' : 'text-warm-500'}>
                  {stepText}
                </span>
              </div>
            );
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="mb-8 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-400">Step {step} of 5</span>
          ...
          <div className="hidden sm:flex items-center space-x-2">
            <span className="text-[11px] text-warm-400 font-medium">Quick Preset:</span>
            <button
              onClick={() => applyPreset('farm')}
              className="px-2.5 py-1 rounded bg-warm-800 hover:bg-warm-700 text-[11px] text-stone-300 transition-colors"
            >
              🌱 AgriTech Demo
            </button>
            <button
        ...
        {/* Step Progress Bar */}
        <div className="w-full bg-warm-900 h-2 rounded-full overflow-hidden flex">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className={`h-full flex-1 transition-all duration-300 ${
                i <= step ? 'bg-gradient-to-r from-emerald-500 to-amber-500' : 'bg-warm-800'
              }`}
            ></div>
          ))}
        </div>
      ...
      {error && (
        <div className="mb-6 p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-400 flex items-center gap-2">
      ...
      {/* Step Contents */}
      <div className="p-6 sm:p-8 rounded-2xl glass-card bg-warm-900/90 border border-warm-800 space-y-6">
        
        {/* STEP 1: Basic Idea */}
        {step === 1 && (
          <div className="space-y-5 animate-fadeIn">
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">Business Idea Title *</label>
              <input
                type="text"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                placeholder="e.g. An AI platform that connects farmers directly with restaurants"
                className="w-full px-4 py-3 rounded-xl bg-warm-950 border border-warm-800 text-sm text-white focus:border-emerald-500 focus:outline-none"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">Detailed Description *</label>
              <textarea
                rows={4}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Industry</label>
                <select
                  value={formData.industry}
                  onChange={(e) => setFormData({ ...formData, industry: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl bg-warm-950 border border-warm-800 text-xs text-white"
                >
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Business Type</label>
                <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Target Region / Country</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Current Stage</label>
                <select
        ...
        {step === 2 && (
          <div className="space-y-5 animate-fadeIn">
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">Who is the target customer?</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">What specific problem are they facing?</label>
              <textarea
                rows={3}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Why would they switch to your solution?</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Problem Frequency</label>
                <select
        ...
        {step === 3 && (
          <div className="space-y-5 animate-fadeIn">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Primary Revenue Model</label>
                <select
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Target Pricing / Customer</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Customer Acquisition Channel</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Product Distribution Channel</label>
                <input
                  type="text"
        ...
        {step === 4 && (
          <div className="space-y-5 animate-fadeIn">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Estimated Startup Budget</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-stone-200">Team Size & Capability</label>
                <input
                  type="text"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">Known Competitors</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-200">Existing Traction (if any)</label>
              <input
                type="text"
        ...
        {step === 5 && (
          <div className="space-y-6 animate-fadeIn">
            <div className="p-4 rounded-xl bg-warm-950 border border-warm-800 space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] uppercase font-bold text-emerald-400 tracking-wide">{formData.industry} • {formData.businessType}</span>
              ...
              <p className="text-xs text-stone-300 leading-relaxed">{formData.description}</p>
              
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-warm-900 text-[11px]">
                <div><span className="text-warm-500">Region:</span> <span className="text-stone-300 font-semibold">{formData.region}</span></div>
                <div><span className="text-warm-500">Revenue:</span> <span className="text-stone-300 font-semibold">{formData.revenueModel}</span></div>
                <div><span className="text-warm-500">Budget:</span> <span className="text-stone-300 font-semibold">{formData.budget}</span></div>
                <div><span className="text-warm-500">Team:</span> <span className="text-stone-300 font-semibold">{formData.teamSize}</span></div>
              </div>
            </div>

            <div className="p-4 rounded-xl bg-emerald-600/10 border border-emerald-500/30 text-xs text-emerald-300 space-y-1">
              <p className="font-bold flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-emerald-400" /> Ready for AI Synthesis Engine
              </p>
        ...
        <div className="pt-4 flex items-center justify-between border-t border-warm-800">
          {step > 1 ? (
            <button
              onClick={handleBack}
              className="px-4 py-2.5 rounded-xl text-xs font-semibold bg-warm-800 hover:bg-warm-700 text-stone-200 transition-colors flex items-center gap-1.5"
            >
          ...
          {step < 5 ? (
            <button
              onClick={handleNext}
              className="px-6 py-2.5 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-900/30 transition-all flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="px-8 py-3.5 rounded-xl text-sm font-extrabold bg-gradient-to-r from-emerald-600 to-amber-600 hover:from-emerald-500 hover:to-amber-500 text-white shadow-xl shadow-emerald-900/40 transition-all transform hover:-translate-y-0.5 flex items-center gap-2"
            >
              🔍 ANALYZE MY IDEA
            </button>
          )}
