...
export const AdminDashboard: React.FC = () => {
  ...
  if (!metrics) {
    return <div className="py-20 text-center text-xs text-slate-400">Loading admin system metrics...</div>;
  }
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8 animate-fadeIn">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white flex items-center gap-2">
          <ShieldCheck className="w-6 h-6 text-purple-400" /> Platform Admin & Analytics Dashboard
        </h1>
        <p className="text-xs text-slate-400">Monitor system performance, user activity, AI API utilization, and industry validation trends</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="p-5 rounded-2xl glass-card border border-slate-800 space-y-1">
          <span className="text-xs text-slate-400 font-semibold">Total Ideas Validated</span>
          <p className="text-3xl font-extrabold text-indigo-400 font-heading">{metrics.totalIdeas.toLocaleString()}</p>
          <p className="text-[11px] text-slate-400">Multi-LLM reports compiled</p>
        </div>

        <div className="p-5 rounded-2xl glass-card border border-slate-800 space-y-1">
          <span className="text-xs text-slate-400 font-semibold">Platform Average Score</span>
          <p className="text-3xl font-extrabold text-amber-400 font-heading">{metrics.averageScore} <span className="text-xs font-normal text-slate-400">/ 100</span></p>
          <p className="text-[11px] text-slate-400">Balanced score distribution</p>
        </div>

        <div className="p-5 rounded-2xl glass-card border border-slate-800 space-y-1">
          <span className="text-xs text-slate-400 font-semibold">AI Infrastructure Health</span>
          ...
          <p className="text-[11px] text-slate-400">Gemini / GPT-4o / Synthesis</p>
        </div>
      </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.popularIndustries} layout="vertical">
                ...
                <Bar dataKey="count" fill="#6366f1" radius={[0, 6, 6, 0]} name="Analyses Count" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
