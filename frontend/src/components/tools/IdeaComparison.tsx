  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400">Select 2 to 4 saved ideas to evaluate metrics and discover the top recommended startup venture</p>
      </div>
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all flex items-center gap-1.5 ${
                  isSel ? 'bg-indigo-600/30 text-indigo-300 border-indigo-500' : 'bg-slate-950 text-slate-400 border-slate-800 hover:text-white'
                }`}
              >
      ...
      {/* Top Winner Card */}
      {recommendedIdea && (
        <div className="p-6 rounded-2xl bg-gradient-to-r from-amber-500/10 via-indigo-600/20 to-emerald-500/10 border border-amber-500/30 space-y-2">
      ...
      {/* Comparison Table */}
      <div className="p-6 rounded-2xl glass-card border border-slate-800 overflow-x-auto">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-800/60 text-slate-300">
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">Industry</td>
              {comparedIdeas.map(i => <td key={i.id} className="py-3 px-3">{i.industry}</td>)}
            </tr>
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">Overall Score</td>
              {comparedIdeas.map(i => (
                <td key={i.id} className="py-3 px-3 font-mono font-bold text-indigo-400 text-sm">{i.score}/100</td>
              ))}
            </tr>
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">AI Recommendation</td>
              {comparedIdeas.map(i => (
                <td key={i.id} className="py-3 px-3 font-bold text-emerald-400">{i.recommendation}</td>
              ))}
            </tr>
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">Market Potential</td>
              {comparedIdeas.map(i => <td key={i.id} className="py-3 px-3 font-mono">{i.analysisResult?.market_score || 85}/100</td>)}
            </tr>
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">Estimated Budget</td>
              {comparedIdeas.map(i => <td key={i.id} className="py-3 px-3">{i.budget || '$25,000'}</td>)}
            </tr>
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">5-Yr Revenue Projection</td>
              {comparedIdeas.map(i => (
            ...
            <tr>
              <td className="py-3 px-3 font-bold text-slate-400">Startup Risk Level</td>
